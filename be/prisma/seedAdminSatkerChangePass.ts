...
async function main() {
  // Ambil semua user dengan role ADMIN SATKER
  const users = await prisma.user.findMany({
    where: {
      userRoles: {
        some: {
          role: {
            roleName: 'admin satuan kerja',
          },
        },
      },
    },
...
function writeToExcel(data: { userId: number; email: string; password: string }[]) {
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'AdminSatkerPasswords');

  const filePath = path.join(__dirname, 'admin-satker-passwords-dokploy.xlsx');
