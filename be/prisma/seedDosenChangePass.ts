...
async function main() {
  ...
  // Baca file Excel
  const filePath = path.join(__dirname, 'dosen-passwords.xlsx');
  ...
  for (const item of data) {
    const user = await prisma.user.findFirst({
      where: {
        ...
        userRoles: {
          some: {
            role: {
              roleName: 'dosen pembimbing magang',
            },
          },
        },
      },
    ...
    if (!user) {
      console.warn(`⚠️ User ${item.email} tidak ditemukan pada dosen.`);
      continue;
    }
  ...
  console.log('🎉 Semua password dosen telah diperbarui.');
}
