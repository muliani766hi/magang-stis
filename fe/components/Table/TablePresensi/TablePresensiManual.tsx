...
import { deletePresensiManual, putSetujuPresensiManual, putTolakPresensiManual } from '@/utils/presensi'
...
const TablePresensiManual
    = ({ records, fetchData }: { records: RecordPerizinan[], fetchData: () => void }) => {
...
const showModal = ({ record, action, fetchData }: { record: RecordPerizinan; action: 'delete'; fetchData: () => void }) => {
    ...
    if (action === 'delete') {
        openConfirmModal({
            ...
            children: (
                <Text size="sm">
                    Apakah Anda yakin ingin menghapus pengajuan presensi manual ini?
                </Text>
            ),
            ...
            onConfirm: () => {
                deletePresensiManual(record.id) // Assuming `record.id` is the parameter required by `putSetujuPerizinan`
                    .then(() => {
                        console.log('setuju');
                        notifications.show({
                            title: 'Berhasil',
                            message: 'Penganjuan presensi berhasil dihapus',
                            color: 'green',
                        });
                        closeAllModals();
                    ...
                    .catch((error) => {
                        notifications.show({
                            title: 'Gagal',
                            message: error.message || 'Terjadi kesalahan saat menghapus pengajuan presensi',
                            color: 'red',
                        });
                    });
            }
...
export default TablePresensiManual
