...
import { getLaporan, postLaporan } from '@/utils/laporan'
...
const LaporanMagang = () => {
    ...
    useEffect(() => {
        if (records && records.laporanId !== form.values.laporanId) {
            form.setValues({
                laporanId: records.laporanId,
    ...
    const handleFileUpload = async (value: any) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/laporan-magang/${mahasiswaId}`, requestOptions);
            if (!response.ok) {
    ...
    const handleUpdateFileUpload = async (value: any) => {
        const token = await getToken();
        const laporanId = value.laporanId;
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/laporan-magang/${laporanId}`, requestOptions);
            if (!response.ok) {
                console.log(await response.json());
                throw new Error('Failed to fetch data');
            }
    ...
    const getFileLaporan = async (laporan: string) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/laporan-magang/download/${laporan}`, requestOptions);
            if (!response.ok) {
    ...
    return (
        <>
            <Text c="dimmed" mb="md">Laporan Magang Final</Text>
            <SimpleGrid cols={{ base: 1, md: 2 }} spacing="lg">
                <div>
                    {isFormSubmitted && (
                        <Alert variant='light' color='green' title='Laporan Magang Final telah terkirim' mb="md" icon={<IconCheck />} />
                    )}
                    ...
                    <Card shadow="xs" padding="md" radius="sm" withBorder>
                        ...
                        <form onSubmit={form.onSubmit(async (values) => {
                            ...
                            <Stack>
                                <DateInput
                                    label="Tanggal"
                                    description="Tanggal pengiriman laporan magang final"
                                    valueFormat='DD-MM-YYYY'
                                    {...form.getInputProps('tanggal')}
                                />
                               
                                <Text size='sm' fw={500}>Laporan Magang Final</Text>
                                <Dropzone
                                    maxSize={20 * 1024 ** 2}
                                ...
                                {isFormSubmitted && (
                                    <Button
                                        ...
                                        onClick={async () => {
                                            getFileLaporan(records.fileLaporan)
                                        }}
                                    >
                                        Download Laporan
                                    </Button>)
                                ...
                                <Group justify='right'>
                                    ...
                                    {/* {isFormSubmitted && (
                                    <Alert variant='light' color='green' title='Laporan magang telah terkirim' icon={<IconCheck />} />
                                )} */}
                                    <Button
                                        type='button'
                                        color="blue"
