...
export async function getLaporan() {
    ...
    const response = await fetch(`${process.env.API_URL}/laporan-magang?mahasiswaId=${mahasiswaId}`,
        {
            method: 'GET',
            headers: {
...
export async function postLaporan(values: any) {
    try {
        const token = cookies().get('token')?.value;
        const mahasiswaId = await getMahasiswaId();
...
export async function getFileLaporan(laporan: string) {
...
export async function putUlasanLaporan(values: any) {
    const token = cookies().get('token')?.value;
